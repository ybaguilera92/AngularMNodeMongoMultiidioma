...
@Component({
    selector     : 'password-activation',
    templateUrl  : './password-activation.component.html',
    encapsulation: ViewEncapsulation.None,
})
export class PasswordActivationComponent implements OnInit, OnDestroy {
            ...
            if (!isNullOrEmpty(params['code'])) {                
                this._authenticationService.recoverPassword(params['code']).subscribe(res => {
                    if (res) {
                        ...
                        this.error("Código de activación incorrecto.");
                     }
                                          
                    },
                    error => {
                        this.error("Código de activación no válido2.");
                    });
            } else {
